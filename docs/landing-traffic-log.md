...
## ⚠️ 既知の制約：ISR キャッシュのため実ユーザーをほとんど捉えられない

記事ページは ISR（`expiration: false`、`src/routes/quiz/[...slug]/+page.server.js`）で
エッジキャッシュされる。**キャッシュヒットしたリクエストはサーバー関数を起動しない**ため、
このフックは呼ばれず、ログにも残らない。

実測（2026-09-03 / 30分間・83件）では次のようになった。
...
ログに残るのはキャッシュ MISS、つまり**ボットが誰も見ていない古い記事URLを掘り起こしたとき**が中心で、
本来の目的（Gunosy 流入の User-Agent を掴む）は果たせない。

**流入元の判別は `docs/traffic-attribution.md` のクライアント側計測に移行した。**
そちらはブラウザで動くのでキャッシュの影響を受けない。
