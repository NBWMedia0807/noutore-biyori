...
本診断のうち、以下を実装しました（`pnpm build` 成功・主要ページのスモーク確認済み）。

- **対応済み**: #1 タイトル二重付与 / #2 JSON-LD 統一 / #3 sitemap 是正 / #4 解答ページ画像最適化 / #5 `/quiz` 文言の実データ化 / #6 未使用 `popular` 削減 / #9 SearchAction 整合 / #10 旧ファイル整理 / #11 未使用コンポーネント削除
- **保留**: #8 固定枠の高さ予約（広告余白の再発リスクを避けるため別途検討）
- **見送り（今回）**: #13 logo.png 圧縮（環境に画像圧縮ツールが無いため次回）
- **未対応（要判断/外部）**: #7 PC 広告枠（効果計測が前提）/ #12 カテゴリルート統合（データ移行が必要）/ #14 themoneytizer 実態確認（外部確認）

> 既知の別件（今回のスコープ外）: `category/[categorySlug]/[quizSlug]` ルートはスタブ非対応のため、`SKIP_SANITY` での `pnpm test`（スモーク）が以前から 503 で失敗する。CI では当該テストは実行されていない。実データ環境では動作する。
