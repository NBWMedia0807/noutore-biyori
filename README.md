...
- サーバー側/スクリプトでのみ `SANITY_READ_TOKEN`/`SANITY_WRITE_TOKEN`（必要に応じて `SANITY_AUTH_TOKEN` を使用）を利用します。
- `SANITY_READ_TOKEN` が未設定の環境では `SANITY_WRITE_TOKEN`、それも無い場合は `SANITY_AUTH_TOKEN` に自動でフォールバックしますが、読み取り専用の権限を利用したい場合は `SANITY_READ_TOKEN` を各環境に設定してください。
...
codex/automate-sanity-schema-deployment-itrh5i

- `SANITY_AUTH_TOKEN` を環境変数として設定した状態で、リポジトリ直下から以下を実行します。
...
- リポジトリの Secrets には `SANITY_AUTH_TOKEN` を登録してください。Sanity の [Manage project tokens](https://www.sanity.io/manage) から **Deploy Studio (Token only)** 権限のトークンを発行し、`Settings > Secrets and variables > Actions > New repository secret` で保存します。シークレットが未設定の場合はログに「SANITY_AUTH_TOKEN is not configured」と表示されます。ワークフローは設定済みのトークンを `SANITY_AUTH_TOKEN` / `SANITY_DEPLOY_TOKEN` に展開し、追加の `sanity login` ステップなしで非対話デプロイを実行します。
